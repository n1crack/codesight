...
pub fn get_commit_heatmap_impl(db: &crate::db::Db, id: i64, year: i32) -> AppResult<HeatmapData> {
    let repo_meta = get_repository_impl(db, id)?;
    let head = current_head(&repo_meta.path);
...
pub fn get_language_breakdown_impl(db: &crate::db::Db, id: i64) -> AppResult<Vec<LanguageStat>> {
    let repo_meta = get_repository_impl(db, id)?;
    let head = current_head(&repo_meta.path);
...
pub fn get_activity_patterns_impl(db: &crate::db::Db, id: i64) -> AppResult<ActivityPatterns> {
    let repo_meta = get_repository_impl(db, id)?;
    let head = current_head(&repo_meta.path);
...
pub fn get_commit_message_stats_impl(
    ...
    let head = current_head(&repo_meta.path);
    let since_dt = since.as_deref().and_then(parse_date);
    let cache_key = format!("messageStats:since={}", since.as_deref().unwrap_or(""));
    cached(db, id, &head, &cache_key, move || {
        let repo = open(&repo_meta.path)?;
...
pub fn list_tags_impl(db: &crate::db::Db, id: i64) -> AppResult<Vec<TagInfo>> {
    ...
    cached(db, id, &head, "tags", move || {
        ...
        for name in names.iter().flatten() {
            ...
            let (tagger_name, timestamp, message) =
                match repo.find_tag(reference.target().unwrap_or_else(|| target_obj.id())) {
                    Ok(tag) => {
...
pub fn get_repos_sparklines_impl(db: &crate::db::Db, days: i64) -> AppResult<Vec<RepoSparkline>> {
    ...
    if let Ok(repo) = GitRepository::open(&repo_meta.path) {
        if let Ok(mut walk) = repo.revwalk() {
            ...
            for oid in walk.flatten() {
                ...
                let date = ts.date_naive();
                let day_diff = (date - cutoff_date).num_days();
...
pub fn list_branches_impl(db: &crate::db::Db, id: i64) -> AppResult<Vec<BranchInfo>> {
    ...
    cached(db, id, &head, "branches", move || {
        ...
        out.sort_by(|a, b| {
            b.is_head.cmp(&a.is_head).then_with(|| {
